...
class wefts {
    ...
    addingWeftsToScene () {
        ...
        for (let i = 0; i < weftNum; i++) {                 // adding weft meshes from weft class to scene                    
            ...
            this.weftZpos.push(zPos);                       // store each zPos of created weft to scene
            zPos = zPos + this.spacing;                     // incrementing zPos by spacing
        }
    }
    // method to get all of the zPos of the wefts
    getWeftsZPos() {
        return this.weftZpos;
    }
}
// class for the warp noodle
class warp {
    constructor (weftsInstance, warpRow, warpMaterial) {
        this.weftsInstance = weftsInstance;
        this.warpRow = warpRow;
        this.warpMaterial = warpMaterial;
    }
    // method to create warp noodle mesh using weft Z pos
    creatingWarpNoodleMesh () {
        const weftZPositions = this.weftsInstance.getWeftsZPos();   // get z positions array from wefts instance input
        let point;
        let warpArrayPoints = [];                               // initializing array to hold all of vec points
        const zPosStart = weftZPositions[0] - 0.2;              // starting zPos to start the warp
        point = new THREE.Vector3(0, 0, zPosStart);             // creating starting 1st point of warp    
        warpArrayPoints.push(point);                            // put point into array
        // for each element in the row -> array points
        for (let k = 0; k < this.warpRow.length; k++) {
            if (this.warpRow[k]==1) { yPos = 0.2; }             // determine the y pos of point
            else { yPos = -0.2; }
            point = new THREE.Vector3(0, yPos, weftZPositions[k]);  // put into z pos and y pos
            warpArrayPoints.push(point);                        // put point into array
        }
        const zPosEnd = weftZPositions[(weftZPositions.length)-1] + 0.2;  // ending zPos to start the warp
        point = new THREE.Vector3(0, 0, zPosEnd);               // creating starting last point of warp    
        warpArrayPoints.push(point);                            // put last point into array
        // array points -> curve
